...
public class PropertyAnalyzer {
  public void setOptionalProperties(List<StructuralVariation> variations) {
    // This comparer will tell us if two properties are the same based on the formal
    // Property comparator
    ...
    // This loop will reduce the list of common properties checking if each property
    // is actually common to all variations
    for (Property prop : commonProps)
      for (StructuralVariation vari : variations)
        if (vari.getProperties().stream().noneMatch(varProp -> {
          return comparer.compare(prop, varProp);
        })) {
    ...
    // For each property, if it is not on the common properties list, flag it as
    // optional.
    for (StructuralVariation vari : variations)
      vari.getProperties().forEach(prop -> {
        if (!commonProps.stream().anyMatch(cProp -> {
          return comparer.compare(prop, cProp);
        }))
