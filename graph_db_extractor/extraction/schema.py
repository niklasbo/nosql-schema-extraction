...
class Schema():
    ...
    def generate(self, path: str, debug=False):
        ...
        if debug:
            print('\n ---- Done Extracting ----')
            Schema.print_grouping(
                {**extracted_grouping_nodes, ** grouping_relationships})

        parsed_list = self._parser.parse(
            extracted_grouping_nodes, grouping_relationships)
