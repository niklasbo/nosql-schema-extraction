...
class Collator():
    ...
    def grouping_nodes(self):
        labels_powerset = self._get_labels_combination(
            self._database.get_labels())
        grouping = dict()
        for label_combination in labels_powerset:
            records = self._database.get_nodes_by_labels(label_combination)
            if len(records) == 0:
                continue
            ...
            for record in records:
                ...
                self._process_relationships_grouping(
                    grouping, key, record['node'].element_id)
    ...
    def grouping_relationships(self):
        ...
        grouping = dict()
        for typed in types:
            records = self._database.get_relationships_by_type(typed)
            if len(records) == 0:
                continue
            ...
            grouping[key] = dict()
            for record in records:
                self._process_props_grouping(
                    grouping, key, record, 'relationship')
    ...
    def _process_relationships_grouping(self, grouping, key, node_id: str):
    ...
    def _get_labels_combination(self, labels: list[str]):
        return self._powerset(labels)

    def _powerset(self, iterable: list[str]):
        "powerset([1,2,3]) --> () (1,) (2,) (3,) (1,2) (1,3) (2,3) (1,2,3)"
        s = list(iterable)
        tuples = list(chain.from_iterable(combinations(s, r)
